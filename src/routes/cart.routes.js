...
router.get("/:userId", async (req, res) => {
  try {
    const [items] = await db.query(
      "SELECT product_id as productId, name, price, image, quantity FROM cart WHERE user_id = ? ORDER BY created_at DESC",
      [req.params.userId]
    );
...
router.post("/add", async (req, res) => {
  ...
  if (!userId || !productId) return res.status(400).json({ success: false, message: "Thiếu thông tin" });
  try {
    await db.query(
